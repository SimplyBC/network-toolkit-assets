...
3. `sections` holds ordered command alternatives. The first successful, recognisable response wins. `documentation` retains the collector's existing per-section collection/merge semantics. Commands must be literal `show ...` or `display ...` reads. Only `| display xml`, `| display json` and `| display set` pipes are allowed. No shell substitutions, redirects, configuration mode or privilege escalation.
...
### Wireless and service recognition

Maintain explicit products in `tools/expand_services.py`. Give each a useful role and official reference. Do not route Cisco WLC to IOS switch commands, Ruckus controllers to FastIron, H3C WX to Comware switch commands, or Huawei AC to VRP switch commands. A shared OS/vendor is not shared collection support. Until an appropriate compiled collector exists, these new products use observed server/infrastructure records without a platform or credentials. The Help catalogue lists the specific product and role.

Run `node tools/check_patterns.mjs` as well as the Python tests. JavaScript Unicode regex compilation rejects some Python-valid escapes, so both runtimes are checked before publication.
