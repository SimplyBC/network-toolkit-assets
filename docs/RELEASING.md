...
2. Increase **both** `version` and monotonically increasing `sequence` in `manifest.json`. Update CHANGELOG. The published 2.0.0 pack uses `engine: 2`, `schemaVersion: 1` and requires the corresponding desktop engine. Do not publish it as an engine-1 update.
